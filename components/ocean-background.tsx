 ...
 *
 * Filosofía:
 *  - Es eficiente: el lienzo se queda quieto y las secciones de la página son
 *    las que pasan por encima al hacer scroll. Un solo canvas para toda la app.
 *  - El hero NO se ve afectado: tiene su propio bg-mesh-deep opaco y queda por
 *    encima de este lienzo.
 *  - Los PageHeader de las páginas internas también son opacos, así que el
...
const TINTS: { body: string; tail: string }[] = [
...
class ShadowFish {
  ...
  private pickTarget(w: number, h: number) {
  ...
  render(ctx: CanvasRenderingContext2D) {
    ...
    const tailUp = {
      x: tail.x + tailDir.x * finReach * 0.5 + finPerp.x * (this.bodyScale * 0.85 + tailWag * 0.3),
      y: tail.y + tailDir.y * finReach * 0.5 + finPerp.y * (this.bodyScale * 0.85 + tailWag * 0.3),
    };
    const tailDown = {
      x: tail.x + tailDir.x * finReach * 0.5 - finPerp.x * (this.bodyScale * 0.85 - tailWag * 0.3),
      y: tail.y + tailDir.y * finReach * 0.5 - finPerp.y * (this.bodyScale * 0.85 - tailWag * 0.3),
...
export function OceanBackground() {
  ...
  return (
    ...
    >
      {/* Color base — abismo puro #061826, idéntico al cierre del hero */}
      <div className="absolute inset-0 bg-abyss" />
      {/* Canvas con peces y plancton — sin gradientes ni overlays decorativos
          sobre él. Cualquier brillo localizado crearía bandas visibles entre
          secciones. La uniformidad cromática es lo que elimina las costuras. */}
