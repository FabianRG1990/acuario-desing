 ...
 */
export function GlassPillCanvas() {
  ...
  useEffect(() => {
    ...
    // Sin env map, sin lights, sin transmission. El cuerpo del pill
    // NUNCA se dibuja porque el fragment shader lo discarea. ───
    const material = new THREE.ShaderMaterial({
      uniforms: {
        rimIntensity: { value: 0.65 },
        topIntensity: { value: 0.50 },
        rimPower: { value: 2.5 },
        topPower: { value: 2.0 },
      },
      vertexShader: /* glsl */ `
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          ...
          // ── Top catchlight: las caras que apuntan hacia arriba
          //    reciben un highlight definido (la luz "del cielo")
          float topDot = max(vWorldNormal.y, 0.0);
          float topness = pow(topDot, topPower);

          // ── Suma de intensidades — el alpha resultante
          float alpha = fresnel * rimIntensity + topness * topIntensity;
          alpha = clamp(alpha, 0.0, 1.0);

          // ── DISCARD: si el alpha es menor a 0.04, no dibujamos nada.
          //    Esto elimina TODO pixel del cuerpo del cristal — pura
          //    transparencia matemática, no hay manera de que aparezca
          //    el gris porque no hay nada dibujado en el body.
          if (alpha < 0.04) discard;

          // Color blanco puro modulado por intensidad → highlights claros
          vec3 color = vec3(1.0);
    ...
    const updateSize = () => {
      ...
      // Camera close enough que el pill llene 95% del canvas (en altura)
      // El 5% restante es buffer para el tilt del pointer (no se sale).
