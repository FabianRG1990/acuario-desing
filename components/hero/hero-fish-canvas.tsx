 ...
 * HeroFishCanvas
 * -----------------------------------------------------------------------------
 * Animación procedural de un pez articulado que persigue al cursor del mouse,
 * más una pequeña escuela ambiental nadando al fondo.
 *
 * Implementación:
 *  - Canvas 2D plano. Sin Three.js ni Framer en este componente para evitar
 *    mezclar engines (regla de la skill).
 *  - El pez tiene una "espina" de 18 segmentos. La cabeza persigue al cursor
 *    con física de muelle (proporcional + damping). Cada segmento posterior
 *    sigue al anterior manteniendo la longitud — es el algoritmo clásico de
 *    procedural creature animation.
 *  - Sobre el espinazo se aplica una oscilación senoidal lateral cuya
 *    amplitud crece con la velocidad. Esto produce el coleo natural.
 *  - El cuerpo se dibuja como dos curvas (lado izquierdo / derecho) cerradas
 *    en cabeza y cola. Se calculan offsets perpendiculares con un perfil de
 *    ancho (aleta dorsal incluida).
 *  - Aletas pectorales y dorsal se dibujan con curvas dependientes del giro.
 *  - Los peces ambientales son mini-instancias con menor tamaño y alpha,
 *    siguiendo blancos circulares — generan profundidad de campo.
 * -----------------------------------------------------------------------------
 */
...
class Fish {
  ...
  update(target: Vec, dt: number) {
    ...
    // Head spring toward target
    const head = this.spine[0];
    const dx = target.x - head.x;
    const dy = target.y - head.y;
    const dist = Math.hypot(dx, dy);

    // Proportional pull, capped speed. Hero fish gets stronger pull.
    ...
    // Track velocity for tail wag amplitude
    this.velocity.x = head.x - this.prev[0].x;
    this.velocity.y = head.y - this.prev[0].y;
    const speed = Math.hypot(this.velocity.x, this.velocity.y);

    // Constrain spine: each follows previous at fixed distance
    ...
    // Lateral oscillation (tail wave). Amplitude scales with speed.
    ...
    for (let i = 2; i < this.spine.length; i++) {
      ...
      // re-constrain length after wave displacement
      const ddx = b.x - a.x;
      const ddy = b.y - a.y;
      const d = Math.hypot(ddx, ddy) || 1;
      b.x = a.x + (ddx / d) * this.segLen;
  ...
  // Body width profile: wider behind head, taper to tail.
  private widthAt(t: number): number {
    // t in [0,1] from head to tail
    const w =
  ...
  render(ctx: CanvasRenderingContext2D) {
    ...
    // ============ CAUDAL FIN (tail) — drawn first so body overlaps base
    const speed = Math.hypot(this.velocity.x, this.velocity.y);
    ...
    const tailUp = {
      x: tail.x + tailDir.x * finReach * 0.55 + finPerp.x * (this.bodyScale * 0.95 + tailWag * 0.4),
      y: tail.y + tailDir.y * finReach * 0.55 + finPerp.y * (this.bodyScale * 0.95 + tailWag * 0.4),
    };
    const tailDown = {
      x: tail.x + tailDir.x * finReach * 0.55 - finPerp.x * (this.bodyScale * 0.95 - tailWag * 0.4),
      y: tail.y + tailDir.y * finReach * 0.55 - finPerp.y * (this.bodyScale * 0.95 - tailWag * 0.4),
    ...
    if (this.isHero) {
      ...
      ctx.quadraticCurveTo(fin1.x, fin1.y, sp2.x + u2.x * this.bodyScale * 0.4, sp2.y + u2.y * this.bodyScale * 0.4);
    ...
    // ============ PECTORAL FIN — small fin near gills
    if (this.isHero) {
      ...
      const finTip = {
        x: sp1.x - u.x * this.bodyScale * 1.4 * flap - (tan.x / tLen) * this.bodyScale * 0.7,
        y: sp1.y - u.y * this.bodyScale * 1.4 * flap - (tan.y / tLen) * this.bodyScale * 0.7,
      };
      ctx.save();
      ctx.fillStyle = this.color.fin;
      ctx.globalAlpha = 0.6;
    ...
    // ============ BODY — smooth path through left + reverse(right)
    ctx.save();
...
// 2D wave equation simulation on a heightfield grid. This is the physics-based
// approximation that real-time water effects use: each cell's next height is
// the average of its 4 neighbours minus its previous height, then damped. The
// cursor "pokes" the field along its path — waves propagate, interfere, and
// reflect at the edges, then are rendered as caustic-style highlights on the
...
class WaveField {
  ...
  // RGB stays static — only alpha is rewritten per frame from the gradient.
  // Cooled silver, very low chroma — reads as moonlight on water rather than
  // colored shimmer, so heavy disturbance never saturates the page.
  private primeColor() {
  ...
  // Add a soft Gaussian-ish bump to the height grid at world coords.
  poke(x: number, y: number, strength: number) {
  ...
  // Sample the line between two cursor positions so fast drags leave a
  // continuous trail of disturbance, not isolated dots.
  pokeLine(x0: number, y0: number, x1: number, y1: number, strength: number) {
    const dx = x1 - x0;
    const dy = y1 - y0;
    const dist = Math.hypot(dx, dy);
  ...
  // Step the wave equation: h_new = (sum_neighbours / 2) - h_prev, damped.
  // Cells are clamped to ±heightCap so a fast cursor cannot pump unbounded
  // energy into the field — caps the visual saturation at the source.
  step() {
    ...
    // Ping-pong the buffers: prev now holds the new state.
    const tmp = this.cur;
    this.cur = this.prev;
    this.prev = tmp;
  }

  // Render the surface: alpha = function of slope magnitude + height. This
  // is the caustic / glint look of real moonlit water.
  render(ctx: CanvasRenderingContext2D, w: number, h: number) {
    ...
    for (let y = 0; y < rows; y++) {
      ...
      for (let x = 0; x < cols; x++) {
        ...
        // Lowered coefficients + lower alpha cap. The slope (gradient) drives
        // the highlight; the absolute height adds a faint body. Together they
        // stay well under 50% peak alpha so the page never feels overlaid.
    ...
    ctx.save();
    // `screen` clamps additive light to 1.0 per channel — unlike `lighter`,
    // overlapping waves cannot push the surface to white. Reads more natural.
...
// Background ambient particles (rising motes / plankton)
class Mote {
  x: number;
  y: number;
  r: number;
...
export function HeroFishCanvas() {
  ...
  useEffect(() => {
    ...
    // Track the last position fed to the wave field so each frame can poke a
    // continuous segment between previous and current cursor positions.
    ...
    // Independent target paths for each ambient (slow lemniscate-style wandering)
    ...
    // Plankton / motes
    const motes = Array.from({ length: 38 }, () => new Mote(w, h));

    // Real water surface — wave-equation simulation
    const waves = new WaveField(w, h);
    const onResizeWave = () => waves.resize(w, h);

    // Idle wander when pointer inactive — use slow figure-8
    let idlePhase = 0;

    let raf = 0;
    let lastT = performance.now();
    // The wave step runs at ~120 Hz independently of the render rate so the
    // simulation stays stable on monitors above 60 Hz and degrades smoothly
    // if the frame rate dips.
    ...
    const tick = (now: number) => {
      ...
      // Inject cursor disturbance into the wave field along the path the
      // cursor traversed since the previous frame. Strength scales gently
      // with speed and is hard-capped — fast drags don't pump runaway energy
      // into the simulation, so the surface stays composed.
      if (pointer.active) {
      ...
      // Step the wave equation in fixed substeps for stability
      waveAccumulator += dt;
      let steps = 0;
      ...
      // Build hero target
      if (pointer.active) {
        // small lead so the fish noses past the cursor
      ...
      // Render plankton (background)
      for (const m of motes) {
        m.update(dt, w, h);
        m.render(ctx);
      }

      // Ambient fish (back layer)
      ...
      // The water surface — drawn LAST, over fish and plankton, because the
      // disturbance lives at the surface above everything else in the tank.
  ...
  return (
    <div
      ...
      aria-hidden
    >
      <div className="pointer-events-none absolute inset-0 caustic-overlay opacity-90" />
      <canvas
        ref={canvasRef}
